    ...
    *,
    ...
    for record in feature_records or []:
        ...
        if dimension_name not in by_dimension:
            continue
        count = _feature_evidence_count(record.get("raw_value"))
        if count <= 0:
            continue
...
def _feature_evidence_count(raw_value: Any) -> int:
    ...
    for key in _EVIDENCE_KEYS:
        ...
        if isinstance(items, list):
            count += sum(1 for item in items if _item_has_evidence(item))
    ...
    if isinstance(snippets, list):
        count += sum(1 for item in snippets if isinstance(item, str) and item.strip())

    insights = raw.get("evidence_insights")
    if isinstance(insights, list):
        count += sum(1 for item in insights if isinstance(item, str) and item.strip())
